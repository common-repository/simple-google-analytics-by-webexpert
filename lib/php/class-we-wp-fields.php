...
class WE_WP_Fields {
  ...
  public function select_input($field) {
   ...
   /**
   * Output a multi select input box.
   *
   * @access public
   * @param array $field
   * @return void
  ...
  public function we_generate_form_field($fields, $common_attrs = array()) {
    if(!empty($fields)) {
  ...
  public function string_wpml($input) {
    if( function_exists( 'icl_register_string' ) ) {
      icl_register_string('Webexpert', ''.$input.'', ''.$input.'');
    }
    if (function_exists( 'icl_t' )) {
      return icl_t('Webexpert', ''.$input.'', ''.$input.'');
    } else {
      return $input;
    }
  }
